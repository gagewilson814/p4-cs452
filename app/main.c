  ...
  while (true) {
    ...
    if (itm) {
      ...
      if (!is_shutdown(pc_queue)) {
        fprintf(stderr,
                "ERROR: Got a null item when queue was not shutdown!\n");
      }
      break;
    }
  }
...
static void usage(char *n) {
  fprintf(stderr,
          "Usage: %s [-c num consumer] [-p num producer] [-i num items] [-s "
          "queue size] <-d introduce delay>\n",
          n);
  fprintf(stderr,
          "-d will introduce a random delay between consumer and producer");
  exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
  ...
  fprintf(stderr,
          "Simulating %d producers %d consumers with %d items per thread and a "
          "queue size of %d\n",
  ...
  // Once all the producers are finished we set a flag so the consumer thread
  // can finish up Once shutdown is called your queue should drain all remaining
  // items and be read for destruction!
